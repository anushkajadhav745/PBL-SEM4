...
//           <div
//             key={item.menuId}
//             className="p-4 shadow-md rounded-xl border border-gray-300 bg-white flex flex-col items-center"
//           >
//             {/* Image Display */}
//             {/* <img
//               src={item.imageUrl || "https://via.placeholder.com/150"}  // Use placeholder if no image URL
//               alt={item.name}
//               className="w-40 h-40 object-cover rounded-lg border border-black mb-4"
//             /> */}

//             <img 
//                 src={item.imageUrl} // Use the image URL received from the backend
//                 alt={item.name} 
...
const MenuPage = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        if (response.data.menu || response.data.menuItems) {
          setMenuItems(response.data.menu || response.data.menuItems);
        } else {
          console.error("Unexpected API response:", response.data);
        }
      })
  ...
  const handleDelete = async () => {
    try {
      ...
      setMenuItems(menuItems.filter((item) => item._id !== deleteItemId));
      toast.success("Menu item deleted successfully");
      setShowDeleteModal(false);
    } catch (error) {
      toast.error("Error deleting menu item");
